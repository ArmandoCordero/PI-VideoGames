...
//A LA API
const infoApi = async() => {
    let url = `https://api.rawg.io/api/games?key=7cc491b83e624f4c8eb36091a327956c`
    let videojuegos = []
    try {
...
const nameApi = async (name) => {
    const infoSearch = await axios.get(`https://api.rawg.io/api/games?search=${name}&key=7cc491b83e624f4c8eb36091a327956c`) 
...
const idApi = async (id) => {
    try {
        const rtaApi = await axios.get(`https://api.rawg.io/api/games/${id}?key=7cc491b83e624f4c8eb36091a327956c`)
        if(rtaApi) {
